...
router.get("/me", async (req, res) => {
    try {
        ...
        const profile = await Creator_1.default.findOne({
...
// ** EXPRESS-VALIDATION CHECKS HERE
    ...
    try {
        let creatorProfile = await Creator_1.default.findOne({ user: req.body.user.id });
        // If found error
        ...
        //Create if not found
        creatorProfile = new Creator_1.default(creatorProfileFields);
...
router.put("/:user_id", [], async (req, res) => {
    ...
    try {
        //Update if found
        const updatedCreator = await Creator_1.default.findOneAndUpdate({ user: req.params.user_id }, { $set: { creatorProfileFields } }, { new: true, runValidators: true });
...
router.patch("/myaccount/:user_id", [], async (req, res) => {
    ...
    try {
        ...
        //Update if found
        const updatedCreator = await Creator_1.default.findOneAndUpdate({ user: userId }, { $set: req.body }, { new: true });
        // Check if the creator was found and updated
        if (!updatedCreator) {
            return res.status(404).json({ msg: "Creator not found" });
        }
...
router.get("/", async (req, res) => {
    try {
        const profiles = await Creator_1.default.find().populate("user", [
            "username",
            "firstName",
            "lastName",
            "avatar",
...
router.get("/username/:username", async (req, res) => {
    try {
        const creator = await Creator_1.default.findOne({
...
router.delete("/:user_id", async (req, res) => {
    try {
        // Remove profile
        await Creator_1.default.findOneAndDelete({ user: req.params.user_id });
        // Remove user
        await User_1.default.findOneAndDelete({ _id: req.params.user_id });
        // Remove user posts
...
router.put("/:user_id/interests", async (req, res) => {
    try {
        ...
        const updatedCreator = await Creator_1.default.findOneAndUpdate({ user: req.params.user_id }, { $set: { interests } }, { new: true });
        // Check if the creator was found and updated
        if (!updatedCreator) {
            return res.status(404).json({ msg: "Creator not found" });
        }
...
router.put("/:user_id/bio", async (req, res) => {
    try {
        const { bio } = req.body;
        const updatedCreator = await Creator_1.default.findOneAndUpdate({ user: req.params.user_id }, { $set: { bio: bio } }, { new: true, runValidators: true });
        // Check if the creator was found and updated
        if (!updatedCreator) {
            return res.status(404).json({ msg: "Creator not found" });
        }
...
router.post("/:user_id/packages", async (req, res) => {
    try {
        const creatorProfile = await Creator_1.default.findOneAndUpdate({ user: req.params.user_id }, { $push: { packages: req.body } }, {
            new: true,
            runValidators: true,
        });
...
router.put("/:user_id/packages", async (req, res) => {
    try {
        ...
        const creatorProfile = await Creator_1.default.findOneAndUpdate({ user: req.params.user_id, "packages._id": updatedPackage._id }, { $set: { "packages.$": updatedPackage } }, {
            new: true,
            runValidators: true,
        });
...
router.delete("/:user_id/packages/:package_id", async (req, res) => {
    try {
        const creatorProfile = await Creator_1.default.updateOne({ user: req.params.user_id, "packages._id": req.params.package_id }, { $pull: { packages: { _id: req.params.package_id } } });
